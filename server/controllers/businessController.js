...
exports.getBusinessesByHitlist = async (req, res) => {
  try {
    const businesses = await Business.find({ hitlistId: req.params.hitlistId })
      .sort({ lastModified: -1 });
    res.json(businesses);
  } catch (error) {
...
exports.createBusiness = async (req, res) => {
  try {
    ...
    // Add business reference to hitlist
    await Hitlist.findByIdAndUpdate(
      req.params.hitlistId,
