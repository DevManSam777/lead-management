...
function initializeCharts() {
    ...
    const observer = new MutationObserver(function (mutations) {
        mutations.forEach(function (mutation) {
            if (mutation.attributeName === "data-theme") {
                // Update colors and options then update charts
                updateAllCharts();
            }
        });
    });
...
function createProjectStatusChart() {
    ...
    // Clear any previous content (important if initializeCharts is called multiple times)
    container.innerHTML = "";
    ...
    try {
        ...
        if (leads.length > 0) {
            // Group leads by status (data preparation remains the same)
            const statusCounts = {
                new: 0,
                contacted: 0,
                "in-progress": 0,
            ...
            // Define initial data and options
            const initialData = {
                labels: ["New", "Contacted", "In Progress", "Won", "Lost"],
                datasets: [
                    {
        ...
        projectStatusChart = null; // Ensure the variable is null on error
    }
}
...
function createNewProjectsChart() {
    ...
    try {
        ...
        if (leads.length > 0) {
             // Group leads by month (data preparation remains the same)
            ...
            container.innerHTML = '<div class="chart-no-data">No projects to display</div>';
            newProjectsChart = null; // Ensure the variable is null if no chart is created
        }
    } catch (error) {
        console.error("Error creating new projects chart:", error);
        container.innerHTML = '<div class="chart-no-data">Error creating chart</div>';
        newProjectsChart = null; // Ensure the variable is null on error
    }
}
...
function createRevenueComparisonChart() {
    ...
    try {
        ...
        if (payments.length > 0) {
            // Group payments by month (data preparation remains the same)
            const monthNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
            const currentDate = new Date();
            const currentYear = currentDate.getFullYear();
            const previousYear = currentYear - 1;
            ...
            console.log("Revenue chart created successfully");
             ...
             revenueComparisonChart = null; // Ensure the variable is null if no chart is created
        }
    } catch (error) {
        console.error("Error creating revenue chart:", error);
        container.innerHTML = '<div class="chart-no-data">Error creating chart</div>';
        revenueComparisonChart = null; // Ensure the variable is null on error
    }
}
...
function updateProjectStatusChart() {
    ...
    // Check if the chart instance exists and if there's data to display
    if (projectStatusChart && leads.length > 0) {
        ...
        projectStatusChart.data.datasets[0].data = [
         ...
         // Update the total projects for the center text plugin (if it exists)
         // This requires re-configuring the plugin or ensuring it accesses the latest data
         // A simpler approach for the center text is to update the plugin options
         // Or, if the plugin directly reads data, ensure that data is updated before .update()
         // For this example, let's assume the plugin correctly accesses the updated data or is simple enough.
         // If the center text plugin needs access to the updated `totalProjects`, you might need to
         // update its options or re-create the plugin config before calling chart.update().
         // For now, we'll rely on the fact that updating chart.data is usually sufficient
         // and the plugin *might* re-calculate based on the updated data during the draw phase.
         // A more robust solution might involve recreating the options object including the plugin.
         // Let's update the options to ensure font sizes and colors are responsive/theme-aware.
        ...
        // --- Update Options (for responsiveness and theme changes) ---
         ...
         // If the center text plugin needs explicit option updates based on the new totalProjects or theme
         // you would update projectStatusChart.options.plugins.centerText here if it had options.
         // Since it accesses totalProjects directly and theme colors via getComputedStyle,
         // simply calling update should trigger a redraw with the latest values.


        // Call update to redraw the chart
        projectStatusChart.update();

        // Ensure no "no data" message is displayed if chart is now visible
        if(container.querySelector('.chart-no-data')) {
             container.innerHTML = ''; // Clear the no data message
             const canvas = document.createElement("canvas"); // Re-add the canvas if it was removed
             container.appendChild(canvas);
             // Note: If the canvas was removed, simply calling update() on the old instance won't work.
             // This highlights why clearing innerHTML for "no data" needs careful handling
             // when trying to update an existing chart instance.
             // A better approach is to hide/show the chart or a message div.
             // Let's revert to a hide/show approach for the no data message for better compatibility.
             // For now, assuming the canvas element was NOT removed if projectStatusChart exists.
             // A more robust solution would manage the visibility of the canvas and a message div.
        }
        ...
        // If chart exists but there's no data, destroy it and show message
        ...
        // If chart doesn't exist and no data, just show message
...
function updateNewProjectsChart() {
    ...
    if (newProjectsChart && leads.length > 0) {
...
function updateRevenueComparisonChart() {
    ...
    if (revenueComparisonChart && payments.length > 0) {
